...
class Element:
    ...
    def __str__(self):
        ...
        return (
            str(self.tag) + " " + self.data + ", ".join([str(c) for c in self.children])
        )
    ...
    def as_tex(self):
        if self.tag == "data":
            ...
            return (
                "\\authorfootnote{"
                + self.children_as_tex().replace("[", "").replace("]", "")
                + "}"
            )
    ...
    def append(self, data):
        if self.tag == "data":
            ...
            len(self.children) > 0
            and self.children[len(self.children) - 1].tag == "data"
        ):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "span" and lang is not None:
            ...
            self.element_stack.append(
                Element(tag=tag, lang=lang, name=name, _class="poem")
            )
            return
        elif tag == "div" and ("class", "line") in attrs:
            self.element_stack.append(
                Element(tag=tag, lang=lang, name=name, _class="line")
            )
            return
