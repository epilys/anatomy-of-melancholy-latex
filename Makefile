INFILES:=$(shell bash -c "grep file citations.bib| cut -d '{' -f2 | cut -d'}' -f1 | sed 's/^/figures\//'")
OUTFILES:=$(shell bash -c "grep file citations.bib| cut -d '{' -f2 | cut -d'}' -f1 | sed 's/^/figures\/thumbs\//'")
